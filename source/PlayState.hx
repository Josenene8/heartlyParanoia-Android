...
class PlayState extends MusicBeatState
{
	...
	var glitchEyes:FlxSprite;
	var grpWeird:FlxTypedGroup<FlxSprite>;
	...
	public var spell:Bool = false;
	public var lmao:Bool = false;
	...
	var glitchEyesLayer:FlxTypedGroup<FlxSprite>;
	// API stuff
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'hartmanns':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/hartmanns/hartmannsDialogue'));
			case 'unknown-x':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/unknown-x/unknown-xDialogue'));
			case 'heart-stirring':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/heart-stirring/heart-stirringDialogue'));
		}
				...
				case 'temple':
					{
						defaultCamZoom = 0.7;
						curStage = 'temple';
						var sky:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('cityWeird/sky', 'koishi'));
						if(FlxG.save.data.antialiasing)
							{
						...
						var city:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('cityWeird/city', 'koishi'));
						city.updateHitbox();
						if(FlxG.save.data.antialiasing)
							{
						...
						var phone:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('cityWeird/phone', 'koishi'));
						phone.updateHitbox();
						if(FlxG.save.data.antialiasing)
							{
						phone.antialiasing = true;
							}
						phone.active = false;
						add(phone);

						var eyes = new FlxSprite(500, 360);
						eyes.frames = Paths.getSparrowAtlas("cityWeird/eyes", "koishi");
						eyes.animation.addByPrefix('idle', 'eyes', 12, true);
						eyes.animation.play('idle');
						add(eyes);

						var tree:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('cityWeird/tree', 'koishi'));
						...
						tree.scrollFactor.set(1.3, 1.3);
						tree.active = false;
						add(tree);
						grpWeird = new FlxTypedGroup<FlxSprite>();
						add(grpWeird);

						var skyW:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('ciudad/sky', 'koishi'));
						skyW.active = false;
						if(FlxG.save.data.antialiasing)
							{
						skyW.antialiasing = true;
							}
						skyW.scrollFactor.set(0.5, 0.5);
						grpWeird.add(skyW);


						var cityW:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('ciudad/city', 'koishi'));
						cityW.active = false;
						cityW.updateHitbox();
						if(FlxG.save.data.antialiasing)
							{
						cityW.antialiasing = true;
							}
						cityW.scrollFactor.set(0.8, 0.8);
						grpWeird.add(cityW);

						var phoneW:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('ciudad/phone', 'koishi'));
						phoneW.active = false;
						phoneW.updateHitbox();
						if(FlxG.save.data.antialiasing)
							{
						phoneW.antialiasing = true;
							}
						grpWeird.add(phoneW);

						var treeW:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('ciudad/tree', 'koishi'));
						treeW.active = false;
						treeW.setGraphicSize(Std.int(tree.width * 1));
						treeW.updateHitbox();
						if(FlxG.save.data.antialiasing)
							{
						treeW.antialiasing = true;
							}
						treeW.scrollFactor.set(1.3, 1.3);
						grpWeird.add(treeW);
					...
					case 'weird':
					{
						...
						var cityforeground:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('cityWeird/cityForeground', 'koishi'));
						cityforeground.updateHitbox();
						cityforeground.antialiasing = true;
						cityforeground.scrollFactor.set(0.7, 0.7);
						cityforeground.active = false;
						add(cityforeground);
						...
						var street:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('cityWeird/street', 'koishi'));
						street.updateHitbox();
				...
				if (FlxG.save.data.distractions)
					{
						...
						if (!PlayStateChangeables.Optimize)
						{
							var weirdTrail = new FlxTrail(dad, null, 4, 24, 0.3, 0.069);
							// evilTrail.changeValuesEnabled(false, false, false, false);
							// evilTrail.changeGraphic()
							add(weirdTrail);
						}
		...
		glitchEyesLayer = new FlxTypedGroup<FlxSprite>();
		add(glitchEyesLayer);
	...
	function realityChangeWeird()
	{
		var sonido = FlxG.sound.play(Paths.sound("koishiRealityEffect", "koishi"),0.4);
		glitchEyes = new FlxSprite(0, 0);
		glitchEyesLayer.clear();
		glitchEyes.frames = Paths.getSparrowAtlas('glitch', 'koishi');
		glitchEyes.animation.addByPrefix('idle', 'glitch', 24, true);
		glitchEyes.scale.set(2, 2);
		glitchEyes.height = 50;
		glitchEyes.width = 50;
		glitchEyes.cameras = [camHUD];
		glitchEyes.animation.play('idle');
		glitchEyesLayer.add(glitchEyes);	
	}
